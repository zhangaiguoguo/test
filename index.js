...
const proxyKEY1 = '__skip'

function getTargetType(value) {
    return value[proxyKEY1] || !Object.isExtensible(value)
...
function _reactiveToValueDeep(target) {
    ...
    function _run(_target) {
        if (_obj.has(_obj)) return
        if (!ordinaryType(_target)) {
            _obj.set(_target, _target)
            try {
...
export const renderScheduler = $$hooks.renderScheduler
export const $useEffectSync = $$hooks.useEffectSync

export const renderMemo = $$hooks.renderMemo
const stateRefTask = new WeakMap();

export function useStateRef(target) {
    try {
...
export function uniteFunction(target) {
    return isFunction(target) ? target : () => target;
}

export function uniteArray(target) {
    return isArray(target) ? target : [target];
}

export function useEffectRef(callback, deps, options = { flush: 'pre' }) {
...
export {
    ...
    effectScope, nextTick, effectReactive, watchEffectSync, propsReactive, triggerRef, EffectScope, toRefsModel, configTool, $$hooks as useHooks, isFunction, isObject, isArray
}